...
def eval_model(dataloaders, device, tokenizer, criterion, model, output_all_results_path = './results_raw/temp.txt' ):
    ...
    output_dir = os.path.dirname(output_all_results_path)
    ...
    rouge = Rouge()
    # TODO: Nicki revised to avoid empty string    
    pred_string_list = [('nicki' if s == '' or s == '.' else s) for s in pred_string_list]
    ...
    P, R, F1 = score(pred_string_list,target_string_list, lang='en', device="cpu", model_type="bert-large-uncased")
...
if __name__ == '__main__': 
    ...
    if 'task1' in task_name:
        dataset_path_task1 = '../datasets/ZuCo/task1-SR/pickle/task1-SR-dataset_wRaw.pickle' 
        with open(dataset_path_task1, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
    if 'task2' in task_name:
        dataset_path_task2 = '../datasets/ZuCo/task2-NR/pickle/task2-NR-dataset_wRaw.pickle' 
        with open(dataset_path_task2, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
    if 'task3' in task_name:
        dataset_path_task3 = '../datasets/ZuCo/task3-TSR/pickle/task3-TSR-dataset_wRaw.pickle' 
        with open(dataset_path_task3, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))
    if 'taskNRv2' in task_name:
        dataset_path_taskNRv2 = '../datasets/ZuCo/task2-NR-2.0/pickle/task2-NR-2.0-dataset_wRaw.pickle' 
    ...
    pretrained_bart = BartForConditionalGeneration.from_pretrained('../EEG2Text_new/bart-large')
