...
class CSCL:
    ...
    def get_triplet(self, Ei, pi, Si, curr_level):
        ...
        for i, (eeg, s, p) in enumerate(zip(Ei, Si, pi)):
            ...
            # TODO: BUGFIX
            if len(E_positive) == 1 and list(E_positive)[0] == eeg:
                pass
            else: 
                E_positive.remove(eeg)
            # >-< Length of [E_positive] before removing [eeg] 1
            # >-< Size of [eeg] = torch.Size([56, 840]), The original sentence is: 
            # "a good case while failing to provide a reason for us to care beyond the very basic dictums of human decency."
            # >-< Diction Info: self.fs.keys[385]
            ...
            E_negative.remove(s)
            # print('>-<'*15, f'  Len of self.S = {len(self.S)} || self.E_negative = {len(E_negative)}')
            E_negative = {e for ss in E_negative for e in self.fs.get(ss, set())}
            # TODO: BUGFIX
            if E_negative.issubset(self.fp[p]):
                pass
            else:
    ...
    def cur_sche(self, curriculums, curr_level):
        ...
        # TODO: Nicki Fixed BUG
        E = E_select[random.randint(0, E_select.shape[0]-1)] if len(E_select) > 1 else E_select[0]
        return E
