...
def get_input_sample(sent_obj, tokenizer, eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], max_len = 56, add_CLS_token = False, subj='unspecified', raw_eeg=False):
    ...
    # TODO: Newly add func_ from DeepRepresentation 
    ...
    for word in sent_obj['word']:
        ...
        # TODO: Newly added attribute from DeepRepresentation 
        if raw_eeg:
            try:
    ...
    # TODO: Newly Added Attribute
    if raw_eeg:
        input_sample['input_raw_embeddings'] = torch.stack(word_raw_embeddings)
    else:
        input_sample['input_raw_embeddings'] = -1000 
    ...
    # TODO: Newly Added Attributes
    input_sample['subject']= subj
    input_sample['sentence'] = target_string

    return input_sample

class ZuCo_dataset(Dataset):
    def __init__(self, input_dataset_dicts, phase, tokenizer, subject = 'ALL', eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], setting = 'unique_sent', is_add_CLS_token = False, raweeg=False):
        self.inputs = []
        self.tokenizer = tokenizer      # BartTokenizer
        ...
        for input_dataset_dict in input_dataset_dicts:
            ...
            if setting == 'unique_sent':
                ...
                # {SR v1-['ZAB', 'ZDM', 'ZGW', 'ZJM', 'ZJN', 'ZJS', 'ZKB', 'ZKH', 'ZKW']},  
                # {SR v2-['YDR', 'YHS', 'YMD', 'YFS', 'YRH', 'YSL', 'YFR', 'YMS', 'YRK', 'YRP', 
                # 'YDG', 'YLS', 'YAC', 'YAK', 'YSD', 'YTL']} for train
                # subject ['ZMG', 'YAG'] for dev
                # subject ['ZPH', 'YIS'] for test
                # TODO: Nicki
                subjectName = list(input_dataset_dict.keys())
                if phase == 'train':
                    print(f'[INFO]initializing a train set using {setting} setting...')
                    for i in range(total_num_sentence):
                        for key in subjectName[:-2]:    # ['ZAB', 'ZDM', 'ZGW', 'ZJM', 'ZJN', 'ZJS', 'ZKB', 'ZKH','ZKW']:
                            input_sample = get_input_sample(input_dataset_dict[key][i],self.tokenizer,eeg_type,bands = bands, add_CLS_token = is_add_CLS_token, subj=key, raw_eeg=raweeg)
                            if input_sample is not None:
                                self.inputs.append(input_sample)
                if phase == 'dev':
                    ...
                    for i in range(total_num_sentence):
                        for key in subjectName[-2:-1]:    # ['ZMG']:
                            input_sample = get_input_sample(input_dataset_dict[key][i],self.tokenizer,eeg_type,bands = bands, add_CLS_token = is_add_CLS_token, subj=key, raw_eeg=raweeg)
                            if input_sample is not None:
                                self.inputs.append(input_sample)
                if phase == 'test':
    ...
    def __getitem__(self, idx):
        ...
        return (
            input_sample['input_embeddings'],       # word-level-eeg
            ...
            input_sample['sent_level_EEG'],         # sent_level
            # TODO: Newly Added Attribute
