...
def eval_model(dataloaders, device, tokenizer, criterion, model, output_all_results_path='./results/test_output.txt'):
    ...
    with open(output_all_results_path, 'w') as f:
        for input_embeddings, seq_len, input_masks, input_mask_invert, target_ids, target_mask, sentiment_labels, sent_level_EEG, raw_eeg, sentence, subject in dataloaders['test']:
            # load in batch
            ...
            # print(f'predicted_string:{predicted_string}')
                ...
                # print(f'logits shape:{logits.shape}, predictions shape:{predictions.shape}')
    ...
    print(f'teacher_forcing {teacher_forcing}, input_noise {intput_noise}')
