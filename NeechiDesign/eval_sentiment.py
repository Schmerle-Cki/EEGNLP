...
def eval_model(dataloaders, device, model, criterion, optimizer, scheduler, num_epochs=25, tokenizer = BartTokenizer.from_pretrained('./bart-large')):
    ...
    for epoch in range(1):
        ...
        for phase in ['test']:
            ...
            # Iterate over data.
            for input_word_eeg_features, seq_lens, input_masks, input_mask_invert, target_ids, target_mask, sentiment_labels, sent_level_EEG, raw_eeg, sentence, subject in dataloaders[phase]:
...
if __name__ == '__main__':
    ...
    if model_name in ['BaselineMLP','BaselineLSTM', 'NaiveFinetuneBert', 'FinetunedBertOnText']:
        ...
        decoder_tokenizer = BartTokenizer.from_pretrained('./bart-large') # Bart
    ...
    if model_name == 'BaselineMLP':
        ...
        model = BartForSequenceClassification.from_pretrained('./bart-large', num_labels=3)
    elif model_name == 'ZeroShotSentimentDiscovery':
        ...
        if classifier_name == 'pretrain_Bert':
            ...
            classifier = BartForSequenceClassification.from_pretrained('./bart-large', num_labels=3)
        elif classifier_name == 'pretrain_RoBerta':
