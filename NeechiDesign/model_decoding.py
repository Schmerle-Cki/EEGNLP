...
class BrainTranslator(nn.Module):
    def __init__(self, pretrained_layers, in_feature=840, decoder_embedding_size=1024, additional_encoder_nhead=8,
        ...
        self.additional_encoder_layer = nn.TransformerEncoderLayer(d_model=in_feature, nhead=additional_encoder_nhead,
                                                                   ...
                                                                   batch_first=True)
        self.EEGEncoder = nn.TransformerEncoder(self.additional_encoder_layer, num_layers=6)
    ...
    def encode_eeg(self, input_embeddings_batch, input_masks_invert):
        ...
        encoded_embedding = self.EEGEncoder(input_embeddings_batch, src_key_padding_mask=input_masks_invert)
    ...
    def generate(
        ...
        encoded_embedding = self.encode_eeg(input_embeddings_batch, input_masks_invert)
    ...
    def forward(self, input_embeddings_batch, input_masks_batch, input_masks_invert, target_ids_batch_converted):
        # embedding of EEG
        eeg_embedding = self.encode_eeg(input_embeddings_batch, input_masks_invert)
        # TODO: Newly Added Attribute
        words_embedding = self.pretrained.model.encoder.embed_tokens(target_ids_batch_converted)
        # print(f'forward-[input_embeddings_batch/input_masks_batch/input_masks_invert/target_ids_batch_converted/encoded_embedding/words_embedding].shape:{input_embeddings_batch.shape}/{input_masks_batch.shape}/{input_masks_invert.shape}/{target_ids_batch_converted.shape}/{eeg_embedding.shape}/{words_embedding.shape}')
        out = self.pretrained(inputs_embeds=eeg_embedding, attention_mask=input_masks_batch,
                              return_dict=True, labels=target_ids_batch_converted)
        return out, eeg_embedding, words_embedding
