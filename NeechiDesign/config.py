...
def get_config(case):
    if case == 'train_decoding': 
        ...
        # TODO: Loss for Alignment
        parser.add_argument('-tempt', '--temperature', type = float, help='temperature for calculating contrastive loss', default = 0.07, required=False)
        parser.add_argument('-alpha', '--criterion_weight', type = float, help='weight for alignment loss calculated by CrossEntropy/MSE', default = 1.0, required=False)
        parser.add_argument('-beta', '--contrastive_weight', type = float, help='weight for calculating contrastive loss', default = 1.0, required=False) # 0.001
        parser.add_argument('-cca_weight', '--cca_weight', type = float, help='weight for canonical correlation analysis', default = 1.0, required=False)
        parser.add_argument('-wd_weight', '--wd_weight', type = float, help='weight for calculating wasserstein distance', default = 1.0, required=False)  # 10.0
